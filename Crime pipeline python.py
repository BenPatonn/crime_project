...
def load_csvs_with_area(directory_path, keyword):
    ...
    try:
        ...
        logging.critical(f"Critical error in {load_csvs_with_area.__name__}: {e}")
...
def merge_on_two_columns(df_1, df_2, col_1_df1, col_2_df1, col_1_df2, col_2_df2, join_type='inner'):
    ...
    """
    try:
        # Validate column names exist in the DataFrames
        if col_1_df1 not in df_1.columns or col_2_df1 not in df_1.columns:
            raise KeyError(f"One or both columns '{col_1_df1}' or '{col_2_df1}' not found in the first DataFrame.")
        if col_1_df2 not in df_2.columns or col_2_df2 not in df_2.columns:
            raise KeyError(f"One or both columns '{col_1_df2}' or '{col_2_df2}' not found in the second DataFrame.")
        ...
        logging.error(f'KeyError in {merge_on_two_columns.__name__}: {ke}')
        df = pd.DataFrame()
    except ValueError as ve:
        logging.error(f'ValueError in {merge_on_two_columns.__name__}: {ve}')
        df = pd.DataFrame()
    except TypeError as te:
        logging.error(f'TypeError in {merge_on_two_columns.__name__}: {te}')
        df = pd.DataFrame()
    except Exception as e:
        logging.error(f'An unexpected error occurred in {merge_on_two_columns.__name__}: {e}')
        df = pd.DataFrame()
